...
class TestData:
    ...
    def test_symbol_to_arcticdb(self, tmp_path):
        data = MyData.pull(["S1", "S2"], shape=(5, 3), columns=["F1", "F2", "F3"]).to_symbol_oriented()

        def _load_and_check_symbol(k, symbol, connection_uri, **kwargs):
            from arcticdb import Arctic

            connection = Arctic(connection_uri)
            lib = connection.get_library("default")
            df = lib.read(symbol).data.squeeze("columns")
            df.index.freq = df.index.inferred_freq
            assert_frame_equal(df, data.data[k])

        connection_uri = "lmdb://" + str(tmp_path / "temp.db")
        data.to_arcticdb(connection_uri)
        _load_and_check_symbol("S1", "S1", connection_uri)
        _load_and_check_symbol("S2", "S2", connection_uri)

        connection_uri1 = "lmdb://" + str(tmp_path / "temp1.db")
        connection_uri2 = "lmdb://" + str(tmp_path / "temp2.db")
        data.to_arcticdb(
            vbt.symbol_dict({"S1": connection_uri1, "S2": connection_uri2}),
            symbol=vbt.symbol_dict({"S1": "T1", "S2": "T2"}),
        )
        _load_and_check_symbol("S1", "T1", connection_uri1)
        _load_and_check_symbol("S2", "T2", connection_uri2)

    def test_feature_to_arcticdb(self, tmp_path):
        data = MyData.pull(["S1", "S2", "S3"], shape=(5, 2), columns=["F1", "F2"]).to_feature_oriented()

        def _load_and_check_feature(k, symbol, connection_uri, **kwargs):
            from arcticdb import Arctic
        ...
        connection_uri = "lmdb://" + str(tmp_path / "temp.db")
        data.to_arcticdb(connection_uri)
        _load_and_check_feature("F1", "F1", connection_uri)
        _load_and_check_feature("F2", "F2", connection_uri)

        connection_uri1 = "lmdb://" + str(tmp_path / "temp1.db")
        connection_uri2 = "lmdb://" + str(tmp_path / "temp2.db")
        data.to_arcticdb(
            vbt.feature_dict({"F1": connection_uri1, "F2": connection_uri2}),
            symbol=vbt.feature_dict({"F1": "T1", "F2": "T2"}),
        )
        _load_and_check_feature("F1", "T1", connection_uri1)
        _load_and_check_feature("F2", "T2", connection_uri2)
    ...
    def test_caching(self, tmp_path):
        data1 = vbt.RandomData.pull(
            start="2021-01-01 UTC",
            end="2021-01-06 UTC",
            cache=True,
            cache_kwargs=dict(cache_dir=tmp_path),
        )
        data2 = vbt.RandomData.pull(
            start="2021-01-01 UTC",
            end="2021-01-06 UTC",
            cache=False,
            cache_kwargs=dict(cache_dir=tmp_path),
        ...
        assert data1 != data4
        data5 = vbt.RandomData.pull(
            start="2021-01-01 UTC",
            end="2021-01-06 UTC",
            cache=True,
            cache_kwargs=dict(cache_dir=tmp_path),
        )
        data6 = vbt.RandomData.pull(
            start="2021-01-02 UTC",
            end="2021-01-07 UTC",
            cache=True,
            cache_kwargs=dict(cache_dir=tmp_path),
        )
        data7 = vbt.RandomData.pull(
            start="2021-01-01 UTC",
            end="2021-01-06 UTC",
            clear_cache=True,
            cache_kwargs=dict(cache_dir=tmp_path),
        )
        data8 = vbt.RandomData.pull(
            start="2021-01-02 UTC",
            end="2021-01-07 UTC",
            cache=True,
            cache_kwargs=dict(cache_dir=tmp_path),
        ...
        assert data9 != data10.rename({"S2": "S1"})

        class RandomData(vbt.RandomData):
            pass

        data11 = vbt.RandomData.pull(
            start="2021-01-01 UTC",
            end="2021-01-06 UTC",
            cache=True,
            cache_kwargs=dict(cache_dir=tmp_path),
        )
        data12 = RandomData.pull(
            start="2021-01-01 UTC",
            end="2021-01-06 UTC",
            cache=True,
            cache_kwargs=dict(cache_dir=tmp_path),
        )
        assert data11 != data12.switch_class(vbt.RandomData)
        data13 = vbt.RandomData.pull(
            ["S1", "S2"],
            start="2021-01-01 UTC",
            end="2021-01-06 UTC",
            start_value=vbt.symbol_dict({"S1": 100, "S2": 200}),
            cache=True,
            clear_cache=True,
            cache_kwargs=dict(cache_dir=tmp_path),
        )
        data14 = vbt.RandomData.pull(
            "S1",
            start="2021-01-01 UTC",
            end="2021-01-06 UTC",
            start_value=100,
        ...
        assert data13.select("S1") == data14
        assert data13.select("S2") == data15
        data16 = vbt.RandomData.pull(
            ["S1", "S2"],
            start="2021-01-01 UTC",
            end="2021-01-06 UTC",
            start_value=vbt.symbol_dict({"S1": 100, "S2": 200}),
            cache=vbt.symbol_dict({"S1": True, "S2": False}),
            clear_cache=True,
            cache_kwargs=dict(cache_dir=tmp_path),
        )
        data17 = vbt.RandomData.pull(
            "S1",
            start="2021-01-01 UTC",
            end="2021-01-06 UTC",
            start_value=100,
        ...
        assert data16.select("S1") == data17
        assert data16.select("S2") != data18
...
class TestCustom:
    ...
    def test_arcticdb_data(self, tmp_path):
        from arcticdb import Arctic

        connection_uri = "lmdb://" + str(tmp_path / "arcticdb")
        connection = Arctic(connection_uri)
        lib = connection.get_library("default", create_if_missing=True)
        sr = pd.Series(np.arange(10), index=pd.date_range("2020", periods=10, tz="utc"), name="hello")
        lib.write("SR", sr)
        arcticdb_data = vbt.ArcticDBData.pull("SR", connection=connection_uri)
        assert_series_equal(arcticdb_data.get(), sr)
        df = pd.DataFrame(np.arange(20).reshape((10, 2)), index=pd.date_range("2020", periods=10, tz="utc"))
        df.columns = pd.Index(["0", "1"], dtype="object")
        lib.write("DF", df)
        arcticdb_data = vbt.ArcticDBData.pull("DF", connection=connection_uri)
        assert_frame_equal(arcticdb_data.get(), df)
        lib2 = connection.get_library("test", create_if_missing=True)
        lib2.write("DF1", df + 1)
        lib2.write("DF2", df + 2)
        arcticdb_data = vbt.ArcticDBData.pull(connection=connection, library="test")
        assert_frame_equal(arcticdb_data.get(symbol="DF1"), df + 1)
        assert_frame_equal(arcticdb_data.get(symbol="DF2"), df + 2)
...
class TestArcticDBDataSaver:
    def test_update(self, tmp_path):
        connection_uri = "lmdb://" + str(tmp_path / "temp.db")
        data = MyData.pull(["S1", "S2"], shape=(5, 3), columns=["F1", "F2", "F3"])

        saver = vbt.ArcticDBDataSaver(
            data,
            save_kwargs=dict(
        ...
        saved_result1 = pd.concat((data.data["S2"].iloc[:-1], updated_data.data["S2"]), axis=0)
        saved_result1.index.freq = "D"
        assert_frame_equal(vbt.ArcticDBData.pull(connection=connection_uri).data["S1"], saved_result0)
        assert_frame_equal(vbt.ArcticDBData.pull(connection=connection_uri).data["S2"], saved_result1)

        new_data = saver.data
        new_saver = vbt.ArcticDBDataSaver(
            new_data,
            save_kwargs=dict(
                connection=connection_uri,
            ),
        ...
        new_saved_result1.index.freq = "D"
        assert_frame_equal(vbt.ArcticDBData.pull(connection=connection_uri).data["S1"], new_saved_result0)
        assert_frame_equal(vbt.ArcticDBData.pull(connection=connection_uri).data["S2"], new_saved_result1)
    ...
    def test_update_every(self, tmp_path):
        ...
        class ArcticDBDataSaver(vbt.ArcticDBDataSaver):
            def update(self, call_count, **kwargs):
                super().update(**kwargs)
                call_count[0] += 1
                if call_count[0] == 5:
        ...
        for i in range(5):
            data = data.update()
        assert_frame_equal(vbt.ArcticDBData.pull(connection=connection_uri).data["S1"], data.data["S1"])
        assert_frame_equal(vbt.ArcticDBData.pull(connection=connection_uri).data["S2"], data.data["S2"])
