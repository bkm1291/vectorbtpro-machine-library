...
from vectorbtpro.utils.path_ import check_mkdir, remove_file, remove_dir
from vectorbtpro.utils.pickling import pdict, RecState, dumps, loads
...
class Data(Analyzable, OHLCDataMixin, metaclass=MetaData):
    ...
    Args:
        ...
        data (KeyDict): Data dictionary structured as feature-oriented (`feature_dict`)
            or symbol-oriented (`symbol_dict`).
        ...
        classes (Optional[KeyDict]): Class definitions for keys.
        level_name (Union[None, bool, MaybeIterable[Hashable]]): Name(s) of levels for keys.
        fetch_kwargs (Optional[KeyDict]): Additional parameters for data fetching.
        returned_kwargs (Optional[KeyDict]): Keyword arguments returned from data fetching.
        last_index (Optional[KeyDict]): Container to record the last datetime index for each key.
        delisted (Optional[KeyDict]): Container to track delisted status for each key.
    ...
    def __init__(
        ...
        classes: tp.Optional[tp.KeyDict] = None,
        level_name: tp.Union[None, bool, tp.MaybeIterable[tp.Hashable]] = None,
        fetch_kwargs: tp.Optional[tp.KeyDict] = None,
        returned_kwargs: tp.Optional[tp.KeyDict] = None,
        last_index: tp.Optional[tp.KeyDict] = None,
        delisted: tp.Optional[tp.KeyDict] = None,
    ...
    @property
    def dict_type(self) -> tp.Type[tp.KeyDict]:
    ...
    @property
    def column_type(self) -> tp.Type[tp.KeyDict]:
        """Column type.
    ...
    def get_keys(self, dict_type: tp.Type[tp.KeyDict]) -> tp.List[tp.Key]:
        """Return keys based on the provided dictionary type.

        Args:
            dict_type (Type[KeyDict]): Data dictionary type used to
    ...
    @property
    def fetch_kwargs(self) -> tp.KeyDict:
        """Keyword arguments originally passed to `Data.fetch_symbol`.
    ...
    @property
    def returned_kwargs(self) -> tp.KeyDict:
        """Keyword arguments returned by `Data.fetch_symbol`.
    ...
    @property
    def last_index(self) -> tp.KeyDict:
        """Last fetched index.
    ...
    @classmethod
    def fix_data_dict_type(cls, data: dict) -> tp.KeyDict:
    ...
    def fix_dict_types_in_kwargs(
        ...
        **kwargs: tp.Kwargs,
        ...
        Args:
            data_type (Type[KeyDict]): Data dictionary type.
    ...
    def from_data(
        ...
        **kwargs,
        ...
        if columns_are_symbols and isinstance(data, symbol_dict):
            raise TypeError("Data cannot have the type symbol_dict when columns_are_symbols is True")
    ...
    def has_key_dict(
        cls,
        arg: tp.Any,
        dict_type: tp.Optional[tp.Type[tp.KeyDict]] = None,
    ) -> bool:
        """Check if the provided argument contains any data dictionary.

        Args:
            arg (Any): Argument to check for a data dictionary.
            dict_type (Optional[Type[KeyDict]]): Dictionary type to validate against.
    ...
    def key_cache_set(
        ...
        The cache key is generated by hashing the combination of the provided key and fetch kwargs.
        The data is serialized and stored in an LMDB database located in the specified cache directory.
        ...
        Args:
            key (Key): Feature or symbol identifier.
            data (SymbolData): Data to be cached.
            fetch_kwargs (KwargsLike): Keyword arguments originally used for fetching data.
            cache_dir (Optional[PathLike]): Directory where the cache database is stored.
            db_name (Optional[str]): Name of the LMDB database.

                If None, a default name based on the class name is used.
        ...
        cache_dir = cls.resolve_base_setting(cache_dir, "cache_dir", sub_path="cache_kwargs")
        db_name = cls.resolve_base_setting(db_name, "db_name", sub_path="cache_kwargs")
        mkdir_kwargs = cls.resolve_base_setting(mkdir_kwargs, "mkdir_kwargs", merge=True, sub_path="cache_kwargs")
        open_kwargs = cls.resolve_base_setting(open_kwargs, "open_kwargs", merge=True, sub_path="cache_kwargs")
        dumps_kwargs = cls.resolve_base_setting(dumps_kwargs, "dumps_kwargs", merge=True, sub_path="cache_kwargs")
        template_context = cls.resolve_base_setting(
            template_context,
            "template_context",
            merge=True,
            sub_path="cache_kwargs",
        )
        if isinstance(cache_dir, CustomTemplate):
            cache_dir = cache_dir.substitute(template_context, eval_id="cache_dir")
        cache_dir = Path(cache_dir)
        check_mkdir(cache_dir, **mkdir_kwargs)
        if db_name is None:
            db_name = cls.__name__ + ".lmdb"
        db_path = cache_dir / db_name
        with Lmdb.open(str(db_path.resolve()), **open_kwargs) as db:
            bytes_ = b""
            bytes_ += dumps(__version__, **dumps_kwargs)
            bytes_ += dumps(cls, **dumps_kwargs)
            bytes_ += dumps(key, **dumps_kwargs)
            bytes_ += dumps(fetch_kwargs, **dumps_kwargs)
            cache_key = hashlib.blake2b(bytes_, digest_size=16).hexdigest()
            db[cache_key] = dumps(data, **dumps_kwargs)
    ...
    def key_cache_get(
        ...
        The cache key is generated by hashing the combination of the provided key and fetch kwargs.
        If a cache entry exists for the generated key, the data is deserialized and returned.
        ...
        db_name = cls.resolve_base_setting(db_name, "db_name", sub_path="cache_kwargs")
        open_kwargs = cls.resolve_base_setting(open_kwargs, "open_kwargs", merge=True, sub_path="cache_kwargs")
        loads_kwargs = cls.resolve_base_setting(loads_kwargs, "loads_kwargs", merge=True, sub_path="cache_kwargs")
        template_context = cls.resolve_base_setting(
            template_context,
            "template_context",
            merge=True,
        ...
        with Lmdb.open(str(db_path.resolve()), **open_kwargs) as db:
            bytes_ = b""
            bytes_ += dumps(__version__, **loads_kwargs)
            bytes_ += dumps(cls, **loads_kwargs)
            bytes_ += dumps(key, **loads_kwargs)
            bytes_ += dumps(fetch_kwargs, **loads_kwargs)
            cache_key = hashlib.blake2b(bytes_, digest_size=16).hexdigest()
            if cache_key in db:
                return loads(db[cache_key], **loads_kwargs)
            return None
    ...
    def key_cache_clear(
        ...
        db_path = cache_dir / db_name
        if db_path.is_file():
            remove_file(db_path, missing_ok=True)
        elif db_path.is_dir():
            remove_dir(db_path, missing_ok=True, with_contents=True)
    ...
    def pull(
        ...
        *,
        ...
        cache: tp.Union[None, bool, tp.KeyDict] = None,
        refresh_cache: tp.Union[bool, tp.KeyDict] = False,
        ...
        **kwargs,
        ...
        Args:
            ...
            cache (Union[None, bool, KeyDict]): Whether to look for cached data before fetching and to cache fetched data.

                Can be a dictionary with keys corresponding to features or symbols.
            refresh_cache (Union[bool, KeyDict]): If True, ignore existing cached data and refresh the cache with newly fetched data.

                Can be a dictionary with keys corresponding to features or symbols.
            clear_cache (bool): If True, clear the entire cache before pulling data.
            cache_kwargs (KwargsLike): Keyword arguments for `Data.key_cache_set` and `Data.key_cache_get`.
        ...
        if isinstance(keys, dict):
            ...
            keys = [keys]
        if not keys:
            raise ValueError("No keys to pull")
        ...
        cache = cls.resolve_base_setting(cache, "cache")
        ...
        if len(cache_kwargs) > 0:
            cache_set_arg_names = get_func_arg_names(cls.key_cache_set)
            cache_get_arg_names = get_func_arg_names(cls.key_cache_get)
            for k, v in cache_kwargs.items():
                if k in cache_set_arg_names:
                    cache_set_kwargs[k] = v
                if k in cache_get_arg_names:
                    cache_get_kwargs[k] = v

        if clear_cache:
            cls.key_cache_clear(**cache_get_kwargs)
        ...
        for i, k in enumerate(keys):
            ...
            if isinstance(cache, key_dict):
                _cache = cls.select_key_from_dict(
                    k,
                    cache,
                    dct_name="cache",
                    dict_type=dict_type,
                ...
                _cache = cache
            if isinstance(refresh_cache, key_dict):
                _refresh_cache = cls.select_key_from_dict(
                    k,
                    refresh_cache,
                    dct_name="refresh_cache",
                    dict_type=dict_type,
                )
            else:
                _refresh_cache = refresh_cache
            if _cache:
                key_cache = cls.key_cache_get(k, fetch_kwargs=key_fetch_kwargs, **cache_get_kwargs)
                if key_cache is not None and not _refresh_cache:
                    outputs[i] = key_cache
                    fetch_kwargs[k] = key_fetch_kwargs
                    continue
            ...
            task_meta.append((i, k))
            fetch_kwargs[k] = key_fetch_kwargs

        if len(tasks) > 0:
            task_keys = [k for _, k in task_meta]
            task_key_index = cls.get_key_index(
                keys=task_keys,
                level_name=level_name,
                feature_oriented=keys_are_features,
            )
            task_results = execute(tasks, size=len(task_keys), keys=task_key_index, **execute_kwargs)
            for (i, k), out in zip(task_meta, task_results):
                outputs[i] = out
                if isinstance(cache, key_dict):
                    _cache = cls.select_key_from_dict(
                        k,
                ...
                if isinstance(refresh_cache, key_dict):
                    ...
                    _refresh_cache = refresh_cache
                if (_cache or _refresh_cache) and out is not None:
                    cls.key_cache_set(k, out, fetch_kwargs=fetch_kwargs[k], **cache_set_kwargs)
            ...
            if out is not None:
                ...
                if _tz_convert is not None:
                    if common_tz_convert is None:
                        ...
                        if not silence_warnings:
                            warn("Returned objects have different timezones (tz_convert). Setting to UTC.")
    ...
    def update(
        ...
        **kwargs,
        ...
        """Create a new `Data` instance with data updated for each feature or symbol.
        ...
        if from_index is None:
            if not silence_warnings:
                if self.feature_oriented:
                    warn("None of the features were updated")
                else:
                    warn("None of the symbols were updated")
            return self.copy()
    ...
    def to_csv(
        self,
        path_or_buf: tp.Union[tp.PathLike, tp.KeyDictTemplate] = ".",
        *,
        ext: tp.Union[str, tp.KeyDictTemplate] = "csv",
        mkdir_kwargs: tp.Union[tp.KwargsLike, tp.KeyDictTemplate] = None,
        check_dict_type: bool = True,
        template_context: tp.KwargsLike = None,
        return_meta: bool = False,
        **kwargs,
    ) -> tp.Optional[tp.KeyDict]:
        ...
        Args:
            path_or_buf (Union[PathLike, KeyDictTemplate]): File path or buffer.

                If a directory is provided, each feature or symbol is saved in a separate file.
            ext (Union[str, KeyDictTemplate]): File extension to use for CSV files when saving multiple files.
            mkdir_kwargs (Union[KwargsLike, KeyDictTemplate]): Keyword arguments for directory creation.
    ...
    def to_hdf(
        ...
        path_or_buf: tp.Union[tp.PathLike, tp.KeyDictTemplate] = ".",
        *,
        key: tp.Union[None, str, tp.KeyDictTemplate] = None,
        mkdir_kwargs: tp.Union[tp.KwargsLike, tp.KeyDictTemplate] = None,
        format: str = "table",
        check_dict_type: bool = True,
        template_context: tp.KwargsLike = None,
        ...
        **kwargs,
        ...
        Args:
            path_or_buf (Union[PathLike, KeyDictTemplate]): File path or buffer.
                ...
                If a directory is provided, each feature or symbol is saved in a separate file.
            key (Union[None, str, KeyDictTemplate]): HDF key under which to store the data.
    ...
    def to_parquet(
        ...
        *,
        mkdir_kwargs: tp.Union[tp.KwargsLike, tp.KeyDictTemplate] = None,
        partition_cols: tp.Union[None, tp.List[str], tp.KeyDictTemplate] = None,
        partition_by: tp.Union[None, tp.AnyGroupByLike, tp.KeyDictTemplate] = None,
        period_index_to: tp.Union[str, tp.AnyGroupByLike, tp.KeyDictTemplate] = "str",
        groupby_kwargs: tp.Union[None, tp.AnyGroupByLike, tp.KeyDictTemplate] = None,
        keep_groupby_names: tp.Union[bool, tp.KeyDictTemplate] = False,
        engine: tp.Union[None, str, tp.KeyDictTemplate] = None,
        check_dict_type: bool = True,
        template_context: tp.KwargsLike = None,
        return_meta: bool = False,
        **kwargs,
        ...
        Args:
            ...
            mkdir_kwargs (Union[KwargsLike, KeyDictTemplate]): Keyword arguments for directory creation.
                ...
                See `vectorbtpro.utils.path_.check_mkdir`.
            partition_cols (Union[None, List[str], KeyDictTemplate]): Column names for partitioning the data.

                Must be None if `partition_by` is provided.
            partition_by (Union[None, AnyGroupByLike, KeyDictTemplate]): Criteria for grouping the index before partitioning.
            ...
            period_index_to (Union[str, AnyGroupByLike, KeyDictTemplate]): Specifies how to convert a PeriodIndex.

                Use "str" to convert periods to strings or provide another method for timestamp conversion.
            groupby_kwargs (Union[None, AnyGroupByLike, KeyDictTemplate]): Keyword arguments for
                `pandas.Series.groupby` and `pandas.Series.resample` methods.

                See `vectorbtpro.base.accessors.BaseIDXAccessor.get_grouper`.
            keep_groupby_names (Union[bool, KeyDictTemplate]): Flag indicating whether to retain original
                group names when partitioning.
            engine (Union[None, str, KeyDictTemplate]): Parquet engine to use; valid options are
                "pyarrow", "fastparquet", or "auto".
            check_dict_type (bool): Flag to validate the type of dictionaries.
            template_context (KwargsLike): Additional context for template substitution.
    ...
    def to_sql(
        self,
        engine: tp.Union[None, str, EngineT, tp.KeyDictTemplate] = None,
        *,
        engine_config: tp.Union[tp.KwargsLike, tp.KeyDictTemplate] = None,
        table: tp.Union[None, str, tp.KeyDictTemplate] = None,
        schema: tp.Union[None, str, tp.KeyDictTemplate] = None,
        to_utc: tp.Union[None, bool, str, tp.Sequence[str], tp.KeyDictTemplate] = None,
        remove_utc_tz: tp.Union[bool, tp.KeyDictTemplate] = True,
        attach_row_number: tp.Union[bool, tp.KeyDictTemplate] = False,
        from_row_number: tp.Union[None, int, tp.KeyDictTemplate] = None,
        row_number_column: tp.Union[None, str, tp.KeyDictTemplate] = None,
        ...
        **kwargs,
    ) -> tp.Union[None, tp.KeyDict, EngineT]:
        ...
        Args:
            engine (Union[None, str, EngineT, KeyDictTemplate]): Database engine instance, a URL string,
                or a mapping for per-feature/symbol configuration.
            engine_config (Union[KwargsLike, KeyDictTemplate]): Additional configuration for the engine.
            table (Union[None, str, KeyDictTemplate]): Table name or mapping for assigning table names to each feature or symbol.
            schema (Union[None, str, KeyDictTemplate]): SQL schema name or mapping; if the schema does not exist, a new one is created.
            to_utc (Union[None, bool, str, Sequence[str], KeyDictTemplate]): Specifies whether to localize or convert datetime fields to UTC.

                See `Data.prepare_dt`.
            remove_utc_tz (Union[bool, KeyDictTemplate]): Indicates whether to remove the timezone after converting to UTC.

                See `Data.prepare_dt`.
            attach_row_number (Union[bool, KeyDictTemplate]): Specifies whether to attach a row number column to the data.
            from_row_number (Union[None, int, KeyDictTemplate]): Starting row number for numbering if a row number column is attached.
            row_number_column (Union[None, str, KeyDictTemplate]): Name of the column to use for row numbers.
        ...
        Returns:
            Union[None, KeyDict, Engine]: Dictionary with metadata if `return_meta` is True
                and/or engine if `return_engine` is True, or None.
        """
        from vectorbtpro.utils.module_ import assert_can_import
    ...
    def to_duckdb(
        self,
        connection: tp.Union[None, str, DuckDBPyConnectionT, tp.KeyDictTemplate] = None,
        *,
        connection_config: tp.Union[tp.KwargsLike, tp.KeyDictTemplate] = None,
        table: tp.Union[None, str, tp.KeyDictTemplate] = None,
        schema: tp.Union[None, str, tp.KeyDictTemplate] = None,
        catalog: tp.Union[None, str, tp.KeyDictTemplate] = None,
        write_format: tp.Union[None, str, tp.KeyDictTemplate] = None,
        write_path: tp.Union[tp.PathLike, tp.KeyDictTemplate] = ".",
        write_options: tp.Union[None, str, dict, tp.KeyDictTemplate] = None,
        mkdir_kwargs: tp.Union[tp.KwargsLike, tp.KeyDictTemplate] = None,
        to_utc: tp.Union[None, bool, str, tp.Sequence[str], tp.KeyDictTemplate] = None,
        remove_utc_tz: tp.Union[bool, tp.KeyDictTemplate] = True,
        if_exists: tp.Union[str, tp.KeyDictTemplate] = "fail",
        check_dict_type: bool = True,
        template_context: tp.KwargsLike = None,
        return_meta: bool = False,
        return_connection: bool = False,
    ) -> tp.Union[None, tp.KeyDict, DuckDBPyConnectionT]:
        ...
        * append: Append the new data to the existing table.
        ...
        Args:
            connection (Union[None, str, DuckDBPyConnection, KeyDictTemplate]): Database connection string or instance.

                If None or a string, a connection is resolved via
                `vectorbtpro.data.custom.duckdb.DuckDBData.resolve_connection`.
            connection_config (Union[KwargsLike, KeyDictTemplate]): Configuration parameters for creating a database connection.
            table (Union[None, str, KeyDictTemplate]): Name of the target table.

                If not provided, the key from the data dictionary is used.
            schema (Union[None, str, KeyDictTemplate]): Name of the schema to use.

                A new schema is created if it does not exist.
            catalog (Union[None, str, KeyDictTemplate]): Name of the catalog to set as default for the connection.
            write_format (Union[None, str, KeyDictTemplate]): Format for writing data to a file.

                Must be "csv", "parquet", or "json" if specified.
            write_path (Union[PathLike, KeyDictTemplate]): File path or directory where data is saved.
            ...
            write_options (Union[None, str, dict, KeyDictTemplate]): Options to pass to the `COPY` command.
            ...
            remove_utc_tz (Union[bool, KeyDictTemplate]): Indicates whether to remove the timezone after converting to UTC.
                ...
                See `Data.prepare_dt`.
            if_exists (Union[str, KeyDictTemplate]): Action to take if the target table already exists.
        ...
        Returns:
            Union[KeyDict, DuckDBPyConnection, None]: Metadata dictionary if
    ...
    def to_arcticdb(
        self,
        connection: tp.Union[None, str, ArcticT, tp.KeyDictTemplate] = None,
        *,
        connection_config: tp.Union[tp.KwargsLike, tp.KeyDictTemplate] = None,
        library: tp.Union[None, str, tp.KeyDictTemplate] = None,
        library_config: tp.Union[tp.KwargsLike, tp.KeyDictTemplate] = None,
        symbol: tp.Union[None, str, tp.KeyDictTemplate] = None,
        method: tp.Union[str, tp.KeyDictTemplate] = "write",
        check_dict_type: bool = True,
        template_context: tp.KwargsLike = None,
        return_meta: bool = False,
        return_connection: bool = False,
        **kwargs,
    ) -> tp.Union[None, tp.KeyDict, ArcticT]:
        """Save data to an ArcticDB database.

        This method saves the internal data to an ArcticDB database. Each feature or symbol in the data
        dictionary is processed individually.

        If `connection` is None or a string, the method resolves a connection using
        `vectorbtpro.data.custom.arcticdb.ArcticDBData.resolve_connection`. The connection is not
        disposed automatically if either `return_meta` or `return_connection` is True.

        Each feature or symbol is persisted to its own symbol in the specified library.
        The symbol name can be overridden by providing a mapping via the `symbol` argument.

        The `method` argument specifies the method to use for saving data, such as "write", "append", or "update".
        See ArcticDB documentation for details on the differences between these methods.

        The library and symbol are created if they do not exist by default, but additional configuration
        can be provided via `connection_config`, `library_config`, and `kwargs` for finer control over the
        connection, library, and saving method.

        Args:
            connection (Union[None, PathLike, Arctic]): ArcticDB connection string, path, or instance.

                See `vectorbtpro.data.custom.arcticdb.ArcticDBData.resolve_connection`.
            connection_config (KwargsLike): Configuration parameters for creating an ArcticDB connection.

                See `vectorbtpro.data.custom.arcticdb.ArcticDBData.resolve_connection`.
            library (Union[None, str, KeyDictTemplate]): Name of the target library in ArcticDB.

                Defaults to "default" if not provided.
            library_config (Union[KwargsLike, KeyDictTemplate]): Configuration parameters for the library.

                See ArcticDB documentation for details.
            symbol (Union[None, str, KeyDictTemplate]): Name of the symbol under which each feature or symbol is saved.
            method (Union[str, KeyDictTemplate]): Method to use for saving data, such as "write", "append", or "update".
            check_dict_type (bool): Flag to validate the type of dictionaries.
            template_context (KwargsLike): Additional context for template substitution.
            return_meta (bool): If True, returns metadata including all processed arguments
                per feature or symbol.
            return_connection (bool): If True and a connection string was provided, returns the
                resolved ArcticDB connection instead of disposing it automatically.
            **kwargs: Keyword arguments for the saving method, such as `arcticdb.Library.write`.

        Returns:
            Union[KeyDict, Arctic, None]: Metadata dictionary if
                `return_meta` is True. The resolved ArcticDB connection is returned if
                `return_connection` is True, otherwise None.
        """
        from vectorbtpro.utils.module_ import assert_can_import

        assert_can_import("arcticdb")
        from vectorbtpro.data.custom.arcticdb import ArcticDBData

        if connection_config is None:
            connection_config = {}
        if (connection is None or isinstance(connection, (str, Path))) and not self.has_key_dict(connection_config):
            connection = ArcticDBData.resolve_connection(connection=connection, **connection_config)
        elif return_connection:
            raise ValueError("Connection can be returned only if URI was provided")
        ...
        for k, v in self.data.items():
            ...
            if _connection is None or isinstance(_connection, (str, Path)):
                _connection = ArcticDBData.resolve_connection(connection=_connection, **_connection_config)
            if symbol is None:
                _symbol = k
            else:
                _symbol = self.resolve_key_arg(
                    symbol,
                    k,
                    "symbol",
                    check_dict_type=check_dict_type,
            ...
            _library = self.resolve_key_arg(
                ArcticDBData.resolve_custom_setting(library, "library"),
                k,
                "library",
                check_dict_type=check_dict_type,
                template_context=_template_context,
            ...
            if _library is None:
                _library = "default"
            _library_config = self.resolve_key_arg(
                library_config,
                k,
                "library_config",
                check_dict_type=check_dict_type,
            ...
            _library_config = merge_dicts(dict(create_if_missing=True), _library_config)
            _method = self.resolve_key_arg(
                ArcticDBData.resolve_custom_setting(method, "method"),
                k,
                "method",
                check_dict_type=check_dict_type,
                template_context=_template_context,
            ...
            _kwargs = self.select_key_kwargs(k, kwargs, check_dict_type=check_dict_type)
            meta[k] = {"symbol": _symbol, "library": _library, **_kwargs}
            lib = _connection.get_library(_library, **_library_config)
            getattr(lib, _method.lower())(_symbol, v, **_kwargs)
    ...
    @classmethod
    def from_arcticdb(cls: tp.Type[DataT], *args, fetch_kwargs: tp.KwargsLike = None, **kwargs) -> DataT:
        """Load data from an ArcticDB database using `vectorbtpro.data.custom.arcticdb.ArcticDBData` and
        switch the object's class.

        Args:
            *args: Positional arguments for `vectorbtpro.data.custom.arcticdb.ArcticDBData.pull`.
            fetch_kwargs (KwargsLike): Keyword arguments originally used for fetching data.

                Refer to the fetching function for further details.
            **kwargs: Keyword arguments for `vectorbtpro.data.custom.arcticdb.ArcticDBData.pull`.
        ...
        if fetch_kwargs is None:
            fetch_kwargs = {}
        data = ArcticDBData.pull(*args, **kwargs)
        data = data.switch_class(cls, clear_fetch_kwargs=True, clear_returned_kwargs=True)
        data = data.update_fetch_kwargs(**fetch_kwargs)
        return data
