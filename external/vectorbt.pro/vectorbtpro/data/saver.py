...
class ArcticDBDataSaver(DataSaver):
    """Class for saving data to an ArcticDB database using `vectorbtpro.data.base.Data.to_arcticdb`."""

    def init_save_data(self, **kwargs) -> None:
        to_arcticdb_kwargs = merge_dicts(
            self.save_kwargs,
            self.init_save_kwargs,
            kwargs,
        )
        self.data.to_arcticdb(**to_arcticdb_kwargs)
        new_index = self.data.wrapper.index
        logger.info(f"Saved initial {len(new_index)} rows from {new_index[0]} to {new_index[-1]}")
