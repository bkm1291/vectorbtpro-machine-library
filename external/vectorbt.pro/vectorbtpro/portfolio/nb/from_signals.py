...
def init_FSLastState_nb(
    ...
    """Initialize `vectorbtpro.portfolio.enums.FSLastState`.
    ...
    Returns:
        FSLastState: Instance of `vectorbtpro.portfolio.enums.FSLastState` with initialized arrays.
    """
...
@register_jitted(cache=True)
def copy_FSLastState_nb(last_state: FSLastState) -> FSLastState:
    """Create a copy of `vectorbtpro.portfolio.enums.FSLastState`.

    Any IDs and indices in the position and order info records are reset to zero.

    Args:
        last_state (FSLastState): Instance of `vectorbtpro.portfolio.enums.FSLastState` to copy.

    Returns:
        FSLastState: Copied instance of `vectorbtpro.portfolio.enums.FSLastState`.
    """
    last_pos_info = last_state.last_pos_info.copy()
    last_limit_info = last_state.last_limit_info.copy()
    last_sl_info = last_state.last_sl_info.copy()
    last_tsl_info = last_state.last_tsl_info.copy()
    last_tp_info = last_state.last_tp_info.copy()
    last_td_info = last_state.last_td_info.copy()
    last_dt_info = last_state.last_dt_info.copy()

    for col in range(len(last_pos_info)):
        if last_pos_info[col]["entry_order_id"] > 0:
            last_pos_info[col]["entry_order_id"] = 0
        if last_pos_info[col]["entry_idx"] > 0:
            last_pos_info[col]["entry_idx"] = 0
        if last_pos_info[col]["exit_order_id"] > 0:
            last_pos_info[col]["exit_order_id"] = 0
        if last_pos_info[col]["exit_idx"] > 0:
            last_pos_info[col]["exit_idx"] = 0
        if last_pos_info[col]["parent_id"] > 0:
            last_pos_info[col]["parent_id"] = 0
    for col in range(len(last_limit_info)):
        if last_limit_info[col]["signal_idx"] > 0:
            last_limit_info[col]["signal_idx"] = 0
        if last_limit_info[col]["creation_idx"] > 0:
            last_limit_info[col]["creation_idx"] = 0
        if last_limit_info[col]["init_idx"] > 0:
            last_limit_info[col]["init_idx"] = 0
    for col in range(len(last_sl_info)):
        if last_sl_info[col]["init_idx"] > 0:
            last_sl_info[col]["init_idx"] = 0
    for col in range(len(last_tsl_info)):
        if last_tsl_info[col]["init_idx"] > 0:
            last_tsl_info[col]["init_idx"] = 0
    for col in range(len(last_tp_info)):
        if last_tp_info[col]["init_idx"] > 0:
            last_tp_info[col]["init_idx"] = 0
    for col in range(len(last_td_info)):
        if last_td_info[col]["init_idx"] > 0:
            last_td_info[col]["init_idx"] = 0
    for col in range(len(last_dt_info)):
        if last_dt_info[col]["init_idx"] > 0:
            last_dt_info[col]["init_idx"] = 0

    return FSLastState(
        last_cash=last_state.last_cash.copy(),
        last_position=last_state.last_position.copy(),
        last_debt=last_state.last_debt.copy(),
        last_locked_cash=last_state.last_locked_cash.copy(),
        last_free_cash=last_state.last_free_cash.copy(),
        last_val_price=last_state.last_val_price.copy(),
        prev_value=last_state.prev_value.copy(),
        last_value=last_state.last_value.copy(),
        last_return=last_state.last_return.copy(),
        last_pos_info=last_pos_info,
        last_limit_info=last_limit_info,
        last_sl_info=last_sl_info,
        last_tsl_info=last_tsl_info,
...
@register_chunkable(
    ...
    arg_take_spec=dict(
        ...
        init_position=base_ch.flex_array_1d_gl_slicer,
        init_price=base_ch.flex_array_1d_gl_slicer,
        ...
        last_state=ch.SequenceTaker(
            [
                base_ch.ArraySlicer(),
                base_ch.array_1d_gl_slicer,
                base_ch.array_1d_gl_slicer,
                base_ch.array_1d_gl_slicer,
                base_ch.ArraySlicer(),
                base_ch.array_1d_gl_slicer,
...
def from_basic_signals_nb(
    ...
    max_order_records: tp.Optional[int] = None,
    max_log_records: tp.Optional[int] = 0,
    last_state: tp.Optional[FOLastState] = None,
) -> SimulationOutput:
    ...
    Args:
        ...
        max_log_records (Optional[int]): Maximum number of log records expected per column.
            ...
            Set to the number of rows in the broadcasted shape if logging is enabled. Set lower to
            reduce memory usage, or higher if multiple logs per timestamp are expected.
        last_state (Optional[FSLastState]): Last simulation state to initialize from.

            See `vectorbtpro.portfolio.enums.FSLastState`.
    ...
    if last_state is None:
        last_state_in = init_FSLastState_nb(
            target_shape=target_shape,
            group_lens=group_lens,
            cash_sharing=cash_sharing,
            init_cash=init_cash,
        ...
        last_state_in = copy_FSLastState_nb(last_state)
    last_cash = last_state_in.last_cash
    last_position = last_state_in.last_position
    last_debt = last_state_in.last_debt
    last_locked_cash = last_state_in.last_locked_cash
    last_free_cash = last_state_in.last_free_cash
    last_val_price = last_state_in.last_val_price
    prev_value = last_state_in.prev_value
    last_value = last_state_in.last_value
    last_return = last_state_in.last_return
    ...
    for group in prange(len(group_lens)):
        ...
        for i in range(_sim_start, _sim_end):
            ...
            if _cash_deposits < 0:
                _cash_deposits = max(_cash_deposits, -max(last_free_cash[group], 0.0))
            ...
            for col in range(from_col, to_col):
                ...
                if _cash_earnings < 0:
                    _cash_earnings = max(_cash_earnings, -max(last_free_cash[group], 0.0))
            ...
            prev_value[group] = last_value[group]
            if save_value:
    ...
    last_state_out = FSLastState(
        ...
        last_value=last_value,
        last_return=last_return,
        last_pos_info=last_state_in.last_pos_info,
        last_limit_info=last_state_in.last_limit_info,
        last_sl_info=last_state_in.last_sl_info,
        last_tsl_info=last_state_in.last_tsl_info,
        last_tp_info=last_state_in.last_tp_info,
        last_td_info=last_state_in.last_td_info,
        last_dt_info=last_state_in.last_dt_info,
    )
        ...
        for i in range(_sim_start, _sim_end):
            ...
            for ci in range(group_len):
                ...
                limit_signal = is_limit_active_nb(
                    ...
                    active=last_limit_info["active"][col],
                )
                if not use_stops:
                ...
                for ci in range(group_len):
                    ...
                    if any_limit_signal:
                        ...
                        if limit_expired_on_open or (not limit_hit_on_open and limit_expired):
                            ...
                            last_limit_info["active"][col] = False
                        else:
                            if limit_hit:
                                exec_limit_set = True
                    ...
                    if (
                        ...
                        if execute_stop:
                            if exec_stop_make_limit:
                                ...
                                last_limit_info["active"][col] = True
                            else:
                        ...
                        if any_stop_signal and not keep_stop:
                            ...
                            last_tsl_info["active"][col] = False
...
@register_chunkable(
    ...
    arg_take_spec=dict(
        ...
        low=base_ch.flex_array_gl_slicer,
        close=base_ch.flex_array_gl_slicer,
        init_cash=RepFunc(portfolio_ch.get_flex_array_1d_cs_slicer),
        init_position=base_ch.flex_array_1d_gl_slicer,
        init_price=base_ch.flex_array_1d_gl_slicer,
        cash_deposits=RepFunc(portfolio_ch.get_flex_array_cs_slicer),
        cash_earnings=base_ch.flex_array_gl_slicer,
        cash_dividends=base_ch.flex_array_gl_slicer,
        ...
        last_state=ch.SequenceTaker(
            [
                RepFunc(portfolio_ch.get_flex_array_1d_cs_slicer),
                base_ch.array_1d_gl_slicer,
                base_ch.array_1d_gl_slicer,
                base_ch.array_1d_gl_slicer,
                RepFunc(portfolio_ch.get_flex_array_1d_cs_slicer),
...
def from_signal_func_nb(  # %? line.replace("from_signal_func_nb", new_func_name)
    ...
    Args:
        ...
        in_outputs (Optional[NamedTuple]): Additional outputs from the simulation.
            ...
            See `vectorbtpro.portfolio.enums.FSSimulationContext.in_outputs`.
        last_state (Optional[FSLastState]): Last simulation state to initialize from.
        ...
        for i in range(_sim_start, _sim_end):
            ...
            if cash_sharing:
                ...
                for col in range(from_col, to_col):
                    ...
                    if _cash_deposits < 0:
                        _cash_deposits = max(_cash_deposits, -max(last_free_cash[col], 0.0))
            ...
            for col in range(from_col, to_col):
                ...
                if cash_sharing:
                    ...
                    if _cash_earnings < 0:
                        _cash_earnings = max(_cash_earnings, -max(last_free_cash[col], 0.0))
