...
def prepare_sim_out_nb(
    ...
    Args:
        ...
        last_state (Optional[NamedTuple]): Optional last state of the simulation.
...
@register_jitted(cache=True)
def is_limit_active_nb(init_idx: int, init_price: float, active: bool) -> bool:
    """Check whether a limit order is active.

    Args:
        init_idx (int): Initial row index; -1 indicates inactivity.
        init_price (float): Initial price; NaN indicates inactivity.
        active (bool): Whether the limit order is marked as active; False indicates inactivity.

    Returns:
        bool: True if the limit order is active, otherwise False.
    """
    return init_idx != -1 and not np.isnan(init_price) and active


@register_jitted(cache=True)
def is_stop_active_nb(init_idx: int, stop: float, active: bool) -> bool:
    """Check whether a stop order is active.

    Args:
        init_idx (int): Initial row index; -1 indicates inactivity.
        stop (float): Stop value; NaN indicates inactivity.
        active (bool): Whether the stop order is marked as active; False indicates inactivity.

    Returns:
        bool: True if the stop order is active, otherwise False.
    """
    return init_idx != -1 and not np.isnan(stop) and active


@register_jitted(cache=True)
def is_time_stop_active_nb(init_idx: int, stop: int, active: bool) -> bool:
    """Check whether a time stop order is active.

    Args:
        init_idx (int): Initial row index; -1 indicates inactivity.
        stop (int): Time stop value; -1 indicates inactivity.
        active (bool): Whether the time stop order is marked as active; False indicates inactivity.

    Returns:
        bool: True if the time stop order is active, otherwise False.
    """
    return init_idx != -1 and stop != -1 and active
...
def is_limit_info_active_nb(limit_info: tp.Record) -> bool:
    ...
    """
    return is_limit_active_nb(limit_info["init_idx"], limit_info["init_price"], limit_info["active"])
...
def is_stop_info_active_nb(stop_info: tp.Record) -> bool:
    ...
    """
    return is_stop_active_nb(stop_info["init_idx"], stop_info["stop"], stop_info["active"])
...
def is_time_stop_info_active_nb(time_stop_info: tp.Record) -> bool:
    ...
    """
    return is_time_stop_active_nb(time_stop_info["init_idx"], time_stop_info["stop"], time_stop_info["active"])
