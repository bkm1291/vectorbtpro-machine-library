...
def init_FOFLastState_nb(
    ...
    """Initialize `vectorbtpro.portfolio.enums.FOFLastState`.
    ...
    Returns:
        FOFLastState: Instance of `vectorbtpro.portfolio.enums.FOFLastState` with initialized arrays.
    """
...
@register_jitted(cache=True)
def copy_FOFLastState_nb(last_state: FOFLastState) -> FOFLastState:
    """Create a copy of `vectorbtpro.portfolio.enums.FOFLastState`.

    Any IDs and indices in the position info records are reset to zero.

    Args:
        last_state (FOFLastState): Instance of `vectorbtpro.portfolio.enums.FOFLastState` to copy.

    Returns:
        FOFLastState: Copied instance of `vectorbtpro.portfolio.enums.FOFLastState`.
    """
...
def from_order_func_nb(  # %? line.replace("from_order_func_nb", new_func_name)
    ...
    last_state: tp.Optional[FOFLastState] = None,
) -> SimulationOutput:
    ...
    Args:
        ...
        in_outputs (Optional[NamedTuple]): Additional outputs from the simulation.
            ...
            See `vectorbtpro.portfolio.enums.SimulationContext.in_outputs`.
        last_state (Optional[FOFLastState]): Last simulation state to initialize from.

            See `vectorbtpro.portfolio.enums.FOFLastState`.
    ...
    if last_state is None:
        last_state_in = init_FOFLastState_nb(
            target_shape=target_shape,
            group_lens=group_lens,
            cash_sharing=cash_sharing,
            init_cash=init_cash,
        ...
        last_state_in = copy_FOFLastState_nb(last_state)
        ...
        for i in range(_sim_start, _sim_end):
            ...
            if track_value:
                ...
                if cash_sharing:
                    ...
                    last_return[group] = returns_nb_.get_return_nb(prev_value[group], last_value[group])
                else:
                    for col in range(from_col, to_col):
                        if last_position[col] == 0:
                        ...
                        last_return[col] = returns_nb_.get_return_nb(prev_value[col], last_value[col])
                ...
                for k in range(group_len):
                    ...
                    if track_value:
                        ...
                        if cash_sharing:
                            ...
                            return_now = returns_nb_.get_return_nb(prev_value[col], value_now - cash_deposits_now)
