...
def last_state_row_stack_func(
    cls: tp.Type["Portfolio"],
    objs: tp.List[tp.ArrayLike],
    *args,
    **kwargs,
) -> tp.ArrayLike:
    """Row-stack multiple state arrays by taking the last state from each.

    Args:
        cls (Type[Portfolio]): Portfolio class.
        objs (List[ArrayLike]): List of state arrays to be row-stacked.
        *args: Additional positional arguments.
        **kwargs: Additional keyword arguments.

    Returns:
        ArrayLike: Row-stacked state array.
    """
    return objs[-1]
...
class MetaPortfolio(type(Analyzable)):
    ...
    @property
    def last_state_config(cls) -> Config:
        """Last state configuration settings.

        Returns:
            Config: Configuration settings for last state.
        """
        return cls._last_state_config
...
class Portfolio(Analyzable, SimRangeMixin, metaclass=MetaPortfolio):
    ...
    Args:
        wrapper (ArrayWrapper): Array wrapper instance.
            ...
            See `vectorbtpro.base.wrapping.ArrayWrapper`.
        sim_out (Union[SimulationOutput, RecordArray]): Simulation output of type
            `vectorbtpro.portfolio.enums.SimulationOutput`, or `order_records`.
        ...
        low (Optional[ArrayLike]): Low price at each bar.
            ...
            Provided in a format that supports flexible indexing.
        order_records (Optional[RecordArray]): Structured NumPy array of order records.
        ...
        last_state (Optional[NamedTuple]): Named tuple containing the last state of the simulation.
        use_last_state (Optional[bool]): Indicates whether to return last state objects when accessing properties.
        ...
        preparer (Optional[BasePFPreparer]): Preparer instance used for preparing the portfolio.
        **kwargs: Keyword arguments for `vectorbtpro.generic.analyzable.Analyzable`.
    """

    _writeable_attrs: tp.WriteableAttrs = {"_in_output_config", "_last_state_config"}
    ...
    _in_output_config: tp.ClassVar[Config] = HybridConfig(
        dict(
            ...
            value=dict(grouping="cash_sharing"),
    ...
    def in_output_config(self) -> Config:
        ...
        !!! note
            To modify in-place outputs, change the configuration in-place, override this property, or
    ...
    _last_state_config: tp.ClassVar[Config] = HybridConfig(
        dict(
            last_cash=dict(
                grouping="cash_sharing",
                obj_type="red_array",
                row_stack_func=last_state_row_stack_func,
            ),
            last_position=dict(
                grouping="columns",
                obj_type="red_array",
    ...
    @property
    def last_state_config(self) -> Config:
        """Last state configuration of `${cls_name}`.
        ...
        !!! note
            To modify last state, change the configuration in-place, override this property, or
            assign a new value to `${cls_name}._last_state_config`.

        Returns:
            Config: Hybrid-copied last state configuration from `${cls_name}._last_state_config`.

                Changing this instance's configuration does not affect the class-level configuration.
        """
        return self._last_state_config

    def __init__(
        self,
        wrapper: ArrayWrapper,
        sim_out: tp.Union[None, enums.SimulationOutput, tp.RecordArray] = None,
        ...
        last_state: tp.Optional[tp.NamedTuple] = None,
        use_last_state: tp.Optional[bool] = None,
        ...
        preparer: tp.Optional[BasePFPreparer] = None,
        **kwargs,
    ) -> None:
        ...
        if sim_out is not None and isinstance(sim_out, enums.SimulationOutput):
            ...
            if last_state is None:
                last_state = sim_out.last_state
        elif sim_out is not None:
            if order_records is None:
                order_records = sim_out
        if close is None:
            raise ValueError("Close must be provided")
        ...
        if use_last_state is None:
            use_last_state = portfolio_cfg["use_last_state"]
        ...
        self._use_last_state = use_last_state
        ...
        self._last_state_config = type(self)._last_state_config.copy()
    ...
    def row_stack_objs(
        ...
        **kwargs,
        ...
        This method stacks `objs` row-wise using either a custom stacking function or an internal
        ...
        raise ValueError(f"Cannot figure out how to stack objects with the name {obj_name!r} along rows")

    @classmethod
    def row_stack_namedtuples(
        cls: tp.Type[PortfolioT],
        namedtuples: tp.Sequence[tp.Optional[tp.NamedTuple]],
        wrappers: tp.Sequence[ArrayWrapper],
        namedtuple_config: tp.KwargsLike = None,
        **kwargs,
    ) -> tp.Optional[tp.NamedTuple]:
        """Stack named tuples along rows.

        This method stacks `namedtuples` row-wise. All named tuples must either be None or
        contain identical fields. For each field, stacking options are determined by merging options from
        `Portfolio.parse_field_options` and `Portfolio.in_output_config`. The stacking is then performed
        on each field using `Portfolio.row_stack_objs`.

        Args:
            namedtuples (Sequence[Optional[NamedTuple]]): Named tuples to be stacked.
            wrappers (Sequence[ArrayWrapper]): Corresponding wrappers for each named tuple.
            namedtuple_config (KwargsLike): Configuration for the named tuple fields.
            **kwargs: Keyword arguments for `Portfolio.row_stack_objs`.

        Returns:
            Optional[NamedTuple]: New named tuple created by stacking fields in `namedtuples`, or
                None if all `namedtuples` are None.
        """
        all_none = True
        ...
        all_keys = set()
        for nt in namedtuples:
            all_keys |= set(nt._asdict().keys())
        for nt in namedtuples:
            if nt is None or len(all_keys.difference(set(nt._asdict().keys()))) > 0:
                raise ValueError("Named tuples to be merged must have the same fields")

        cls_dir = set(dir(cls))
        new_namedtuples = {}
        for field in namedtuples[0]._asdict().keys():
            field_options = merge_dicts(
                cls.parse_field_options(field),
                namedtuple_config.get(field, None),
            )
            ...
            new_field_obj = cls.row_stack_objs(
                [getattr(nt, field) for nt in namedtuples],
                [wrapper for wrapper in wrappers],
                **_kwargs,
            )
            new_namedtuples[field] = new_field_obj

        return type(namedtuples[0])(**new_namedtuples)

    @classmethod
    def row_stack_in_outputs(
        cls: tp.Type[PortfolioT],
        in_outputs: tp.Sequence[tp.Optional[tp.NamedTuple]],
        wrappers: tp.Sequence[ArrayWrapper],
        **kwargs,
    ) -> tp.Optional[tp.NamedTuple]:
        """Stack in-place outputs along rows.

        This method stacks `in_outputs` row-wise. All in-place outputs must either be None or
        contain identical fields. For each field, stacking options are determined by merging options from
        `Portfolio.parse_field_options` and `Portfolio.in_output_config`. The stacking is then performed
        on each field using `Portfolio.row_stack_objs`.

        Args:
            in_outputs (Sequence[Optional[NamedTuple]]): In-place output named tuples to be stacked.
            wrappers (Sequence[ArrayWrapper]): Corresponding wrappers for each in-place output.
            **kwargs: Keyword arguments for `Portfolio.row_stack_objs`.

        Returns:
            Optional[NamedTuple]: New in-place output created by stacking fields in `in_outputs`, or
                None if all `in_outputs` are None.
        """
        return cls.row_stack_namedtuples(
            in_outputs,
            wrappers,
            namedtuple_config=cls.in_output_config,
            **kwargs,
        )

    @classmethod
    def row_stack_last_states(
        cls: tp.Type[PortfolioT],
        last_states: tp.Sequence[tp.Optional[tp.NamedTuple]],
        wrappers: tp.Sequence[ArrayWrapper],
        **kwargs,
    ) -> tp.Optional[tp.NamedTuple]:
        """Stack last states along rows.

        This method stacks `last_states` row-wise. All last states must either be None or
        contain identical fields. For each field, stacking options are determined by merging options from
        `Portfolio.parse_field_options` and `Portfolio.last_state_config`. The stacking is then performed
        on each field using `Portfolio.row_stack_objs`.

        Args:
            last_states (Sequence[Optional[NamedTuple]]): In-place output named tuples to be stacked.
            wrappers (Sequence[ArrayWrapper]): Corresponding wrappers for each last state.
            **kwargs: Keyword arguments for `Portfolio.row_stack_objs`.

        Returns:
            Optional[NamedTuple]: New last state created by stacking fields in `last_states`, or
                None if all `last_states` are None.
        """
        return cls.row_stack_namedtuples(
            last_states,
            wrappers,
            namedtuple_config=cls.last_state_config,
            **kwargs,
        )
    ...
    def row_stack(
        ...
        *objs: tp.MaybeSequence[PortfolioT],
        ...
        last_state_kwargs: tp.KwargsLike = None,
        **kwargs,
    ) -> PortfolioT:
        ...
        benchmark close, cash deposits, cash earnings, and call sequences.

        In-place outputs and last states are merged using `Portfolio.row_stack_in_outputs` and
        `Portfolio.row_stack_last_states` respectively, and records are aggregated
        using `vectorbtpro.records.base.Records.row_stack_records_arrs`.
        ...
        !!! note
            If portfolio chaining is enabled, `init_cash`, `init_position`, and `init_price` are set
            to the first object's values, and subsequent objects' values are disregarded, as the chaining
            order is expected to reflect the simulation chronology. In this case, `combine_init_cash`,
            `combine_init_position`, and `combine_init_price` have no effect.

            If portfolio chaining is disabled, when possible, avoid including initial position and price in
            portfolios to be stacked, as their stacking order might not correctly reflect the simulation
            chronology and could lead to inaccurate results.
        ...
        Args:
            *objs (MaybeSequence[Portfolio]): (Additional) `Portfolio` instances to stack.
            ...
            chained (Optional[bool]): Whether portfolios are chained, meaning that the simulation of each
                portfolio starts with the last state of the previous one.

                If None, it is inferred based on the presence of `last_state` in the preparer of each portfolio.
            ...
            in_output_kwargs (KwargsLike): Keyword arguments for stacking in-place outputs.

                See `Portfolio.row_stack_in_outputs`.
            last_state_kwargs (KwargsLike): Keyword arguments for stacking last state.

                See `Portfolio.row_stack_last_states`.
        ...
        if chained is None:
            chained = True
            for i, obj in enumerate(_objs[1:]):
                if obj.preparer is None:
                    chained = False
                    break
                target_args = obj.preparer.target_args
                if target_args is None or target_args.get("last_state", None) is None:
                    chained = False
                    break
        ...
        if "order_records" not in kwargs:
            orders_cls = None
            for obj in _objs:
                if orders_cls is not None and obj.orders_cls != orders_cls:
                    raise ValueError("Objects to be merged must have the same orders_cls")
                orders_cls = obj.orders_cls
            kwargs["order_records"] = orders_cls.row_stack_records_arrs(
                *[obj.orders for obj in _objs],
                **kwargs,
            )
        if "log_records" not in kwargs:
            logs_cls = None
            for obj in _objs:
                if logs_cls is not None and obj.logs_cls != logs_cls:
                    raise ValueError("Objects to be merged must have the same logs_cls")
                logs_cls = obj.logs_cls
            kwargs["log_records"] = logs_cls.row_stack_records_arrs(
                *[obj.logs for obj in _objs],
                **kwargs,
            )
        if "init_cash" not in kwargs:
            ...
            if stack_init_cash_objs:
                ...
                if stack_init_cash_objs and not chained:
        ...
        if "init_position" not in kwargs:
            ...
            if stack_init_position_objs and not chained:
        ...
        if "init_price" not in kwargs:
            ...
            if stack_init_price_objs and not chained:
        ...
        if "in_outputs" not in kwargs:
            kwargs["in_outputs"] = cls.row_stack_in_outputs(
                [obj._in_outputs for obj in _objs],
                [obj.wrapper for obj in _objs],
                **{**kwargs, **resolve_dict(in_output_kwargs)},
            )
        if "last_state" not in kwargs:
            kwargs["last_state"] = cls.row_stack_last_states(
                [obj._last_state for obj in _objs],
                [obj.wrapper for obj in _objs],
                **{**kwargs, **resolve_dict(last_state_kwargs)},
            )
        if "preparer" not in kwargs:
            if chained:
                kwargs["preparer"] = _objs[-1].preparer
            else:
                kwargs["preparer"] = None
    ...
    def column_stack_objs(
        ...
        **kwargs,
        ...
        for obj in objs:
            if obj is None or isinstance(obj, bool) or (checks.is_np_array(obj) and obj.size == 0):
                if not checks.is_deep_equal(obj, objs[0]):
                    raise ValueError(f"Cannot unify scalar objects with the name {obj_name!r}")
            else:
                all_none = False
                break
        if all_none:
        ...
        raise ValueError(f"Cannot figure out how to stack objects with the name {obj_name!r} along columns")

    @classmethod
    def column_stack_namedtuples(
        cls: tp.Type[PortfolioT],
        namedtuples: tp.MaybeSequence[tp.Optional[tp.NamedTuple]],
        wrappers: tp.Sequence[ArrayWrapper],
        namedtuple_config: tp.KwargsLike = None,
        **kwargs,
    ) -> tp.Optional[tp.NamedTuple]:
        """Stack named tuples along columns.

        Merge `namedtuples` by stacking corresponding fields.
        All named tuples must either be None or have identical fields.
        ...
        and `namedtuple_config`. Stacking for each field is performed using `Portfolio.column_stack_objs`.

        Args:
            namedtuples (MaybeSequence[Optional[NamedTuple]]): Named tuples to be stacked.
            wrappers (Sequence[ArrayWrapper]): Corresponding wrappers for each named tuple.
            namedtuple_config (KwargsLike): Configuration for the named tuple fields.
        ...
        Returns:
            Optional[NamedTuple]: New named tuple created by stacking fields in `namedtuples`, or
                None if all `namedtuples` are None.
        """
        if len(namedtuples) == 1:
            namedtuples = namedtuples[0]
        namedtuples = list(namedtuples)
        if namedtuple_config is None:
            namedtuple_config = {}
    ...
    def column_stack_in_outputs(
        cls: tp.Type[PortfolioT],
        in_outputs: tp.MaybeSequence[tp.Optional[tp.NamedTuple]],
        wrappers: tp.Sequence[ArrayWrapper],
        **kwargs,
    ) -> tp.Optional[tp.NamedTuple]:
        """Stack in-place outputs along columns.

        Merge `in_outputs` by stacking corresponding fields.
        ...
        Args:
            in_outputs (MaybeSequence[Optional[NamedTuple]]): In-place outputs to be stacked.
            wrappers (Sequence[ArrayWrapper]): Wrappers corresponding to each in-place output.
            **kwargs: Keyword arguments for `Portfolio.column_stack_objs`.
        ...
        Returns:
            Optional[NamedTuple]: New in-place output created by stacking fields in `in_outputs`, or
                None if all `in_outputs` are None.
        """
        return cls.column_stack_namedtuples(
            in_outputs,
            wrappers,
            namedtuple_config=cls.in_output_config,
            **kwargs,
    ...
    @classmethod
    def column_stack_last_states(
        cls: tp.Type[PortfolioT],
        last_states: tp.MaybeSequence[tp.Optional[tp.NamedTuple]],
        wrappers: tp.Sequence[ArrayWrapper],
        **kwargs,
    ) -> tp.Optional[tp.NamedTuple]:
        """Stack last states along columns.

        Merge `last_states` by stacking corresponding fields.
        All last state tuples must either be None or have identical fields.

        If a field is also defined as an attribute of the `Portfolio` class, its options are used
        to determine the required type and layout by merging results from `Portfolio.parse_field_options`
        and `Portfolio.last_state_config`. Stacking for each field is performed using `Portfolio.column_stack_objs`.

        Args:
            last_states (MaybeSequence[Optional[NamedTuple]]): In-place outputs to be stacked.
            wrappers (Sequence[ArrayWrapper]): Wrappers corresponding to each last state.
            **kwargs: Keyword arguments for `Portfolio.column_stack_objs`.
    ...
    def column_stack(
        ...
        * One-dimensional arrays are concatenated via `vectorbtpro.base.wrapping.ArrayWrapper.concat_arrs`.
        ...
        In-place outputs and last states are merged using `Portfolio.column_stack_in_outputs` and
        `Portfolio.column_stack_last_states` respectively, and records are aggregated
        ...
        if "order_records" not in kwargs:
            ...
            for obj in _objs:
                if orders_cls is not None and obj.orders_cls != orders_cls:
                    raise ValueError("Objects to be merged must have the same orders_cls")
                orders_cls = obj.orders_cls
            kwargs["order_records"] = orders_cls.column_stack_records_arrs(
                *[obj.orders for obj in _objs],
                **kwargs,
            )
        if "log_records" not in kwargs:
            ...
            for obj in _objs:
                if logs_cls is not None and obj.logs_cls != logs_cls:
                    raise ValueError("Objects to be merged must have the same logs_cls")
                logs_cls = obj.logs_cls
            kwargs["log_records"] = logs_cls.column_stack_records_arrs(
                *[obj.logs for obj in _objs],
                **kwargs,
            )
        if "init_cash" not in kwargs:
        ...
        if "in_outputs" not in kwargs:
            kwargs["in_outputs"] = cls.column_stack_in_outputs(
                [obj._in_outputs for obj in _objs],
                [obj.wrapper for obj in _objs],
                **{**kwargs, **resolve_dict(in_output_kwargs)},
            )
        if "last_state" not in kwargs:
            kwargs["last_state"] = cls.column_stack_last_states(
                [obj._last_state for obj in _objs],
                [obj.wrapper for obj in _objs],
                **{**kwargs, **resolve_dict(last_state_kwargs)},
            )
    ...
    def get_namedtuple_obj(
        ...
        **kwargs,
        ...
        """Find and wrap an named tuple object matching a specified field.

        This method searches for a named tuple field that matches the provided name or any of its aliases.
        If the field is found among the attributes of this `Portfolio` instance, its options are used to
        determine the required type and layout. Otherwise, options are resolved from `namedtuple_config`,
        and candidates are filtered using `Portfolio.matches_field_options`.
        The identified named tuple object is then wrapped via `Portfolio.wrap_obj`.

        Args:
            namedtuple (NamedTuple): Named tuple to search within.
            field (str): Field identifier.
            namedtuple_config (KwargsLike): Configuration for the named tuple fields.
            wrapper (Optional[ArrayWrapper]): Array wrapper instance.
        ...
        Returns:
            Union[None, bool, AnyArray]: Wrapped named tuple object.
        """
        if namedtuple_config is None:
            namedtuple_config = {}
        ...
        for _field in set(namedtuple._fields):
            _field_options = merge_dicts(
                self.parse_field_options(_field),
                namedtuple_config.get(_field, None),
            )
            if (not found_attr and field == _field) or (
                ...
                if found_field is not None:
                    raise ValueError(f"Multiple fields for {field!r} found in named tuple")
                found_field = _field
                found_field_options = _field_options
        if found_field is None:
            raise AttributeError(f"No compatible field for {field!r} found in named tuple")
        obj = getattr(namedtuple, found_field)
        if found_attr and checks.is_np_array(obj) and obj.shape == (0, 0):
            return None
        kwargs = merge_dicts(
            dict(
    ...
    def get_in_output(
        ...
        **kwargs,
        ...
        return self.get_namedtuple_obj(
            self.in_outputs,
            field,
            namedtuple_config=self.in_output_config,
            wrapper=wrapper,
            group_by=group_by,
            **kwargs,
        )
    ...
    def get_last_state(
        ...
        **kwargs,
        ...
        """Find and wrap an last state object matching a specified field.

        This method searches for an last state field that matches the provided name or any of its aliases.
        If the field is found among the attributes of this `Portfolio` instance, its options are used to
        determine the required type and layout. Otherwise, options are resolved from `Portfolio.last_state`
        and `Portfolio.last_state_config`, and candidates are filtered using `Portfolio.matches_field_options`.
        The identified last state object is then wrapped via `Portfolio.wrap_obj`.
        ...
        Returns:
            Union[None, bool, AnyArray]: Wrapped last state object.
        """
        if self.last_state is None:
            raise ValueError("No last state attached")

        return self.get_namedtuple_obj(
            self.last_state,
            field,
            namedtuple_config=self.last_state_config,
            wrapper=wrapper,
            group_by=group_by,
            **kwargs,
        )
    ...
    def namedtuple_indexing_func(
        ...
        **kwargs,
        ...
        """Perform indexing on a named tuple.

        Processes each field in the named tuple by determining the field's configuration
        from attribute options, parsed field options via `Portfolio.parse_field_options`, and
        the indexing configuration in `namedtuple_config`. Each field is then indexed
        using the `Portfolio.index_obj` method.

        Args:
            namedtuple (NamedTuple): Named tuple to index.
            wrapper_meta (dict): Metadata from the indexing operation on the wrapper.
            namedtuple_config (KwargsLike): Configuration for the named tuple fields.
            **kwargs: Keyword arguments for `Portfolio.index_obj`.

        Returns:
            NamedTuple: New named tuple with the indexed named tuple fields.
        """
        if namedtuple_config is None:
            namedtuple_config = {}

        new_namedtuple = {}
        for field, obj in namedtuple._asdict().items():
            field_options = merge_dicts(
            ...
            new_namedtuple[field] = new_obj
        return type(namedtuple)(**new_namedtuple)
    ...
    def in_outputs_indexing_func(self, wrapper_meta: dict, **kwargs) -> tp.Optional[tp.NamedTuple]:
        ...
        Returns:
            Optional[NamedTuple]: New in-place output created from indexing `Portfolio.in_outputs`,
                or None if `Portfolio.in_outputs` is None.
        """
        if self.in_outputs is None:
            return None

        return self.namedtuple_indexing_func(
            self.in_outputs,
            wrapper_meta,
            namedtuple_config=self.in_output_config,
            **kwargs,
    ...
    def last_state_indexing_func(self, wrapper_meta: dict, **kwargs) -> tp.Optional[tp.NamedTuple]:
        """Perform indexing on `Portfolio.last_state`.

        Processes each field in `Portfolio.last_state` by determining the field's configuration
        from attribute options, parsed field options via `Portfolio.parse_field_options`, and
        the indexing configuration in `Portfolio.last_state_config`. Each field is then indexed
        using the `Portfolio.index_obj` method.
        ...
        Returns:
            Optional[NamedTuple]: New last state created from indexing `Portfolio.last_state`,
                or None if `Portfolio.last_state` is None.
        """
        if self.last_state is None:
            return None
        ...
        **kwargs,
        ...
        Args:
            *args: Positional arguments for `vectorbtpro.base.wrapping.ArrayWrapper.indexing_func_meta`.
            ...
            last_state_kwargs (KwargsLike): Keyword arguments for indexing last state.

                See `Portfolio.last_state_indexing_func`.
        ...
        new_last_state = _self.last_state_indexing_func(wrapper_meta, **resolve_dict(last_state_kwargs))
    ...
    def resample_namedtuple(
        ...
        **kwargs,
        ...
        """Resample a named tuple using the provided resampler.

        For each field in the named tuple, this method merges options obtained from
        `Portfolio.parse_field_options` with those from `namedtuple_config` and then
        applies the resampling function via `Portfolio.resample_obj`.

        Args:
            namedtuple (NamedTuple): Named tuple to resample.
            resampler (Union[Resampler, PandasResampler]): `vectorbtpro.base.resampling.base.Resampler` instance
                or a Pandas resampler.
            namedtuple_config (KwargsLike): Configuration for the named tuple fields.
        ...
        Returns:
            NamedTuple: Resampled named tuple created from the existing named tuple.
        """
        new_in_outputs = {}
        for field, obj in namedtuple._asdict().items():
            field_options = merge_dicts(
        ...
        return type(namedtuple)(**new_in_outputs)
    ...
    def resample_in_outputs(
        ...
        **kwargs,
        ...
        Returns:
            Optional[NamedTuple]: New in-place output created from resampling `Portfolio.in_outputs`,
                or None if `Portfolio.in_outputs` is not set.
        """
        if self.in_outputs is None:
            return None

        return self.resample_namedtuple(
            self.in_outputs,
            resampler,
            namedtuple_config=self.in_output_config,
            **kwargs,
    ...
    def resample_last_state(
        self,
        resampler: tp.Union[Resampler, tp.PandasResampler],
        **kwargs,
    ) -> tp.Optional[tp.NamedTuple]:
        """Resample `Portfolio.last_state` using the provided resampler.

        For each field in `Portfolio.last_state`, this method merges options obtained from
        `Portfolio.parse_field_options` with those from `Portfolio.last_state_config` and then
        applies the resampling function via `Portfolio.resample_obj`.
        ...
        Returns:
            Optional[NamedTuple]: New last state created from resampling `Portfolio.last_state`,
                or None if `Portfolio.last_state` is not set.
        """
        if self.last_state is None:
            return None
        ...
        **kwargs,
        ...
        Args:
            *args: Positional arguments for `vectorbtpro.base.wrapping.ArrayWrapper.resample_meta`.
            ...
            last_state_kwargs (KwargsLike): Keyword arguments for resampling last state.

                See `Portfolio.resample_last_state`.
        ...
        if _self._in_outputs is not None:
            ...
            new_in_outputs = None
        if _self._last_state is not None:
            new_last_state = _self.resample_last_state(resampler, **resolve_dict(last_state_kwargs))
        else:
            new_last_state = None
    ...
    def from_orders(
        ...
        last_state: tp.Optional[tp.MappingLike] = None,
        ...
        **kwargs,
        ...
        Args:
            ...
            last_state (Optional[tp.MappingLike]): Mapping of last state objects available via
                `Portfolio.last_state` as a named tuple.

                !!! note
                    For Numba versions below 0.54, `last_state` must be a globally defined named tuple
            ...
            attach_preparer (bool): If True, attaches the preparer instance to the portfolio instance.
        ...
        if isinstance(close, FOPreparer):
            ...
            del local_kwargs["attach_preparer"]
        ...
        if return_sim_out:
            return sim_out
        if not attach_preparer:
            preparer = None
        return cls(sim_out=sim_out, preparer=preparer, **prep_result.pf_args)
    ...
    def update(
        self: PortfolioT,
        close: tp.Union[tp.ArrayLike, OHLCDataMixin, BasePFPreparer, PFPrepResult],
        attach_preparer: bool = True,
        return_preparer: bool = False,
        return_prep_result: bool = False,
        return_sim_out: bool = False,
        stack: bool = True,
        **kwargs,
        ...
        """Create a new `Portfolio` instance with updated data or parameters.

        !!! note
            Returns a new `Portfolio` instance instead of modifying the current portfolio in place.

        Args:
            close (Union[ArrayLike, OHLCDataMixin, BasePFPreparer, PFPrepResult]): Close prices or
                OHLC data used for portfolio simulation.

                Broadcasts.

                * If an instance of `vectorbtpro.data.base.OHLCDataMixin`, extracts open, high, low, and close prices.
                * If an instance of `vectorbtpro.portfolio.preparing.BasePFPreparer`, it is used as a preparer.
                * If an instance of `vectorbtpro.portfolio.preparing.PFPrepResult`, it is used as a preparer result.
            attach_preparer (bool): If True, attaches the preparer instance to the portfolio instance.
            return_preparer (bool): If True, returns the preparer instance (of the same type as `Portfolio.preparer`).
            ...
            return_sim_out (bool): If True, returns the simulation output
                (`vectorbtpro.portfolio.enums.SimulationOutput`).
            stack (bool): Whether to row-stack existing portfolio with updated/new portfolio.
            **kwargs: Keyword arguments for preparing the portfolio.
        ...
        if preparer is None:
            raise ValueError(
                "Cannot update without a preparer. "
                "Use `attach_preparer=True` in simulation methods, or set `preparer` manually."
            )
        if isinstance(close, type(preparer)):
            preparer = close
            prep_result = None
        elif isinstance(close, PFPrepResult):
            preparer = None
            ...
            if parsed_data is not None:
                ...
                kwargs["close"] = close
            if "last_state" not in kwargs:
                kwargs["last_state"] = self.last_state
            preparer = preparer.replace(**kwargs)
            if not return_preparer:
                preparer.set_seed()
            prep_result = None
        if return_preparer:
        ...
        if not attach_preparer:
            preparer = None
        pf = type(self)(sim_out=sim_out, preparer=preparer, **prep_result.pf_args)
        if stack:
            return type(self).row_stack((self, pf), chained=True, preparer=preparer)
        return pf
    ...
    def regroup(self: PortfolioT, group_by: tp.GroupByLike, **kwargs) -> PortfolioT:
        if self.cash_sharing:
            if self.wrapper.grouper.is_grouping_modified(group_by=group_by):
                raise ValueError("Cannot modify grouping globally when cash_sharing is True")
    ...
    @property
    def use_last_state(self) -> bool:
        """Whether to return last state objects when calling properties.

        Returns:
            bool: True if last state objects should be used for property values; otherwise, False.
        """
        return self._use_last_state

    @property
    def last_state(self) -> tp.Optional[tp.NamedTuple]:
        """Named tuple with last state objects.

        Returns:
            Optional[NamedTuple]: Named tuple containing last state objects, or None if not set.
        """
    ...
    @property
    def preparer(self) -> tp.Optional[BasePFPreparer]:
        """Preparer instance used for preparing the portfolio.

        Returns:
            Optional[BasePFPreparer]: Preparer instance used for portfolio preparation, or
                None if the portfolio was not prepared, or if the preparer was not attached.
        """
    ...
    @classmethod
    def build_last_state_config_doc(cls, source_cls: tp.Optional[type] = None) -> str:
        """Build last state configuration documentation.
        ...
        Returns:
            str: Generated last state configuration documentation.
        """
        if source_cls is None:
            source_cls = Portfolio
        return string.Template(inspect.cleandoc(get_dict_attr(source_cls, "last_state_config").__doc__)).substitute(
            {"last_state_config": cls.last_state_config.prettify_doc(), "cls_name": cls.__name__},
        )

    @classmethod
    def override_last_state_config_doc(cls, __pdoc__: dict, source_cls: tp.Optional[type] = None) -> None:
        """Override the last state configuration documentation in the `__pdoc__` dictionary.
        ...
        Returns:
            None
        """
        __pdoc__[cls.__name__ + ".last_state_config"] = cls.build_last_state_config_doc(source_cls=source_cls)


Portfolio.override_in_output_config_doc(__pdoc__)
Portfolio.override_last_state_config_doc(__pdoc__)
