...
base_arg_config = ReadonlyConfig(
    dict(
        ...
        last_state=dict(has_default=False),
    )
)
"""_"""
...
class BasePFPreparer(BasePreparer):
    ...
    @cachedproperty
    def pre__last_state(self) -> tp.Union[None, tp.NamedTuple, CustomTemplate]:
        """Argument `last_state` before broadcasting.

        Returns:
            Union[None, NamedTuple, CustomTemplate]: Last state before broadcasting.
        """
        last_state = self["last_state"]
        if (
            last_state is not None
            and not isinstance(last_state, CustomTemplate)
            and not checks.is_namedtuple(last_state)
        ):
            last_state = to_field_mapping(last_state)
            last_state = namedtuple("LastState", last_state)(**last_state)
        return last_state
    ...
    def template_context(self) -> tp.Kwargs:
        return merge_dicts(
            dict(
                ...
                last_state=self.pre__last_state,
            ),
    ...
    def last_state(self) -> tp.Optional[tp.NamedTuple]:
        """Template-substituted last state.

        Substitutes templates in the last state using the template context.

        Returns:
            Optional[NamedTuple]: Last state after template substitution.
        """
        return substitute_templates(self.pre__last_state, self.template_context, eval_id="last_state")
...
fs_arg_config = ReadonlyConfig(
    dict(
        ...
        pre_order_segment_func_nb=dict(),
        pre_order_segment_args=dict(type="args", substitute_templates=True),
...
class FSPreparer(BasePFPreparer):
    ...
    def pre_order_segment_func_nb(self) -> tp.Optional[tp.Callable]:
        ...
        """
        if self.dynamic_mode:
            if self["pre_order_segment_func_nb"] is None:
                return nb.no_pre_func_nb
            return self["pre_order_segment_func_nb"]
        return None
            ...
            if self.dynamic_mode:
                ...
                if self["pre_order_segment_func_nb"] is not None:
                    self.adapt_staticized_to_udf(
                        staticized, 
                        self["pre_order_segment_func_nb"], 
                        "pre_order_segment_func_nb",
                    )
    ...
    def template_context(self) -> tp.Kwargs:
        return merge_dicts(
            dict(
                ...
                pre_order_segment_func_nb=self.pre_order_segment_func_nb,
                pre_order_segment_args=self.pre__pre_order_segment_args,
...
class FDOFPreparer(FOFPreparer):
    ...
    def order_func_nb(self) -> tp.Callable:
        ...
        order_func_nb = self["order_func_nb"]
        if self.flexible and order_func_nb is not None:
            raise ValueError("Argument order_func_nb cannot be provided when flexible is True")
        if order_func_nb is None:
    ...
    def flex_order_func_nb(self) -> tp.Callable:
        ...
        if not self.flexible and flex_order_func_nb is not None:
            raise ValueError("Argument flex_order_func_nb cannot be provided when flexible is False")
        if flex_order_func_nb is None:
