...
class SimulationOutput(tp.NamedTuple):
    ...
    last_state: tp.Optional[tp.NamedTuple]
...
"""
__pdoc__[
    "SimulationOutput.last_state"
] = """Named tuple representing the last state of the simulation.
...
If False, `SimulationContext.last_val_price`, `SimulationContext.prev_value`,
`SimulationContext.last_value`, and `SimulationContext.last_return` will remain NaN,
and open position statistics will not be updated. Consequently, `SizeType.Value`,
`SizeType.TargetValue`, and `SizeType.TargetPercent` cannot be used.
...
"""
__pdoc__[
    "SimulationContext.prev_value"
] = """Previous value per column (or per group with cash sharing).
...
class FOLastState(tp.NamedTuple):
    last_cash: tp.Array1d
    last_position: tp.Array1d
    last_debt: tp.Array1d
    last_locked_cash: tp.Array1d
...
__pdoc__["FOLastState"] = "Named tuple representing the last state for simulation based on orders."
for field in FOLastState._fields:
    if field in SimulationContext._fields:
        __pdoc__["FOLastState." + field] = f"See `SimulationContext.{field}`."
for field in FOLastState._fields:
    if "FOLastState." + field not in __pdoc__:
        raise KeyError(f"Documentation for FOLastState.{field} is missing")


class FSLastState(tp.NamedTuple):
    last_cash: tp.Array1d
    last_position: tp.Array1d
    last_debt: tp.Array1d
    last_locked_cash: tp.Array1d
...
__pdoc__["FSLastState"] = "Named tuple representing the last state for simulation based on signals."
for field in FSLastState._fields:
    if field in FSSimulationContext._fields:
        __pdoc__["FSLastState." + field] = f"See `FSSimulationContext.{field}`."
for field in FSLastState._fields:
    if "FSLastState." + field not in __pdoc__:
        raise KeyError(f"Documentation for FSLastState.{field} is missing")


class FOFLastState(tp.NamedTuple):
    last_cash: tp.Array1d
    last_position: tp.Array1d
    last_debt: tp.Array1d
    last_locked_cash: tp.Array1d
...
__pdoc__["FOFLastState"] = "Named tuple representing the last state for simulation based on order functions."
for field in FOFLastState._fields:
    if field in SimulationContext._fields:
        __pdoc__["FOFLastState." + field] = f"See `SimulationContext.{field}`."
for field in FOFLastState._fields:
    if "FOFLastState." + field not in __pdoc__:
        raise KeyError(f"Documentation for FOFLastState.{field} is missing")
...
Fields:
    ...
    active: Flag indicating if the order is currently active.
"""
