...
class CacheableRegistry(Base):
    ...
    def match_setups(
        ...
        if not collapse:
            ...
            if "class" in kind:
                class_matches = set()
                for class_setup in list(self.class_setups.values()):
                    for q in query_like:
            ...
            if "instance" in kind:
                for instance_setup in list(self.instance_setups.values()):
                    if instance_setup not in collapse_setups:
                        for q in query_like:
                            if q.matches_setup(instance_setup):
                                ...
                                break
            if "unbound" in kind:
                for unbound_setup in list(self.unbound_setups.values()):
                    if unbound_setup not in collapse_setups:
                        for q in query_like:
                            if q.matches_setup(unbound_setup):
                                ...
                                break
            if "runnable" in kind:
                for run_setup in list(self.run_setups.values()):
...
CABaseSetupT = tp.TypeVar("CABaseSetupT", bound="CABaseSetup")
...
class CABaseSetup(CAMetrics, DefineMixin):
    ...
    @property
    def same_type_setups(self: CABaseSetupT) -> tp.List[CABaseSetupT]:
        ...
        Returns:
            List[CABaseSetupT]: List of setups that are of the same type as this setup.
        """
        raise NotImplementedError
...
class CAClassSetup(CABaseDelegatorSetup, DefineMixin):
    ...
    * `CAInstanceSetup` for instance setups.
    ...
    def instance_setups(self) -> tp.Set["CAInstanceSetup"]:
        ...
        """
        matches = set()
        for instance_setup in list(self.registry.instance_setups.values()):
    ...
    @property
    def same_type_setups(self: CAClassSetupT) -> tp.List[CAClassSetupT]:
        return list(self.registry.class_setups.values())
...
class CAInstanceSetup(CABaseDelegatorSetup, DefineMixin):
    ...
    def run_setups(self) -> tp.Set["CARunSetup"]:
        ...
        if self.contains_garbage:
            return set()
        matches = set()
        for run_setup in list(self.registry.run_setups.values()):
    ...
    @property
    def same_type_setups(self: CAInstanceSetupT) -> tp.List[CAInstanceSetupT]:
        return list(self.registry.instance_setups.values())
...
class CAUnboundSetup(CABaseDelegatorSetup, DefineMixin):
    ...
    @property
    def same_type_setups(self: CAUnboundSetupT) -> tp.List[CAUnboundSetupT]:
        return list(self.registry.unbound_setups.values())
...
class CARunSetup(CABaseSetup, DefineMixin):
    ...
    @property
    def hits(self) -> int:
        return sum([run_result.hits for run_result in list(self.cache.values())])
    ...
    @property
    def total_size(self) -> int:
        return sum([run_result.result_size for run_result in list(self.cache.values())])

    @property
    def total_elapsed(self) -> tp.Optional[timedelta]:
        total_elapsed = None
        for run_result in list(self.cache.values()):
    ...
    @property
    def same_type_setups(self: CARunSetupT) -> tp.List[CARunSetupT]:
        return list(self.registry.run_setups.values())
