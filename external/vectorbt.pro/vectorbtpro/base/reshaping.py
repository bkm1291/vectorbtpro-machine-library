                        ...
                        if new_index is None:
                            ...
                            if index_from.lower() == "strict":
                                raise ValueError(
                                    ...
                                    f"is not allowed when {index_str}_from is 'strict'"
                                )
    ...
    if new_index is not None:
        if maxlen > len(new_index):
            if isinstance(index_from, str) and index_from.lower() == "strict":
                raise ValueError(f"Broadcasting {index_str} is not allowed when {index_str}_from is 'strict'")
    ...
    * Mapping with values for each object and a special key `_def` for the default.
    ...
    for i, k in enumerate(all_keys):
        ...
        if k in param_keys:
            ...
            if _axis == 0:
                raise ValueError("Parameters do not support broadcasting with axis is 0")
            obj = param_product[k]
            new_obj = []
