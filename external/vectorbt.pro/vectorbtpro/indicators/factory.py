    ...
    def run_pipeline(
        ...
        **kwargs,
        ...
        if random_subset is not None:
            if per_column:
                raise ValueError("Cannot select random subset when per_column is True")
        ...
        if use_raw is not None:
            ...
            if isinstance(return_raw, str):
                if return_raw.lower() == "outputs":
                    if use_run_unique and not silence_warnings:
                        warn("Raw outputs are produced by unique parameter combinations when run_unique is True")
                    return outputs
                else:
            ...
            if kwargs.get("return_cache", False):
                if use_run_unique and not silence_warnings:
                    warn("Cache is produced by unique parameter combinations when run_unique is True")
                return outputs
            ...
            if input_shape_2d is not None:
                if n_input_cols != input_shape_2d[1]:
                    if per_column:
                        raise ValueError(
                            "All outputs must have the same number of columns as inputs when per_column is True"
                        )
                    else:
                        raise ValueError(
        ...
        def custom_func(
            ...
            **kwargs_,
            ...
            if jitted_loop and not checks.is_numba_func(apply_func):
                raise ValueError("Apply function must be Numba-compiled when jitted_loop is True")
            if skipna and len(in_output_tuple) > 1:
                raise ValueError("NaNs cannot be skipped for in-place outputs")
            if skipna and jitted_loop:
                raise ValueError("NaNs cannot be skipped when jitted_loop is True")
            if forward_skipna:
            ...
            if skipna and len(_input_tuple) > 0:
                ...
                if any_nan:
                    ...
                    if "post_execute_func" in execute_kwargs:
                        raise ValueError("Cannot use custom post_execute_func when skipna is True")
                    execute_kwargs["post_execute_func"] = _post_execute_func
                    if "post_execute_kwargs" in execute_kwargs:
                        raise ValueError("Cannot use custom post_execute_kwargs when skipna is True")
                    ...
                    if "post_execute_on_sorted" in execute_kwargs:
                        raise ValueError("Cannot use custom post_execute_on_sorted when skipna is True")
