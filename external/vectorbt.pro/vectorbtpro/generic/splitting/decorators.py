    ...
    def decorator(func: tp.Callable) -> tp.Callable:
        ...
        def wrapper(*args, **kwargs) -> tp.Any:
            ...
            def apply_wrapper(*_args, __template_context: tp.KwargsLike = None, **_kwargs) -> tp.Any:
                try:
                    range_key = (__template_context["split_idx"], __template_context["set_idx"])
                    if __template_context is None:
                        __template_context = {}
                    __template_context = dict(__template_context)
                    __template_context["grid_results_map"] = grid_results_map
                    ...
                    if __template_context["set_idx"] == 0:
                        try:
                            ...
                            grid_results_map[range_key] = grid_results
                        except Exception as e:
                            if skip_errored or isinstance(e, NoResultsException):
                                grid_results_map[range_key] = NoResult
                            raise e
                        train_results = grid_results_map[range_key]
                    else:
                        train_results = grid_results_map[(range_key[0], 0)]
                    if train_results is NoResult:
                        if raise_no_results:
                    ...
                    result = parameterized_func(
                        *_args,
                        ...
                        _template_context=dict(grid_results=train_results),
                        **_kwargs,
                    )
                    if return_grid is not None:
                        if return_grid.lower() == "first":
                            return train_results, result
