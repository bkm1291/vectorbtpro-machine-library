...
def wait_until(dt: tp.DatetimeLike, **kwargs) -> None:
    """Wait until the specified datetime.

    Args:
        dt (DatetimeLike): Datetime representation (string, number, or datetime-like object).
        **kwargs: Keyword arguments for `vectorbtpro.utils.datetime_.to_timestamp`.

    Returns:
        None
    """
    dt = dt.to_timestamp(dt, **kwargs)
...
def wait(
    freq: tp.FrequencyLike,
    from_dt: tp.Optional[tp.DatetimeLike] = None,
    floor: bool = False,
    log: bool = False,
    readable: bool = True,
    freq_kwargs: tp.KwargsLike = None,
    timestamp_kwargs: tp.KwargsLike = None,
    humanize_kwargs: tp.KwargsLike = None,
) -> None:
    """Wait for the specified frequency duration.

    Args:
        freq (FrequencyLike): Frequency representation (string, offset, or timedelta).
        from_dt (Optional[DatetimeLike]): Starting datetime from which to calculate the wait duration.

            If None, defaults to the current datetime.
        floor (bool): If True, wait until the next frequency boundary from `from_dt`;
            otherwise, wait for the duration of the frequency starting from `from_dt`.

            For example, if `freq` is 1h and `from_dt` is 10:15, with `floor=True`
            it will wait until 11:00, while with `floor=False` it will wait until 11:15.
        log (bool): If True, log the waiting duration and the target time to wait until.
        readable (bool): If True, use human-readable format for logging.
        freq_kwargs (KwargsLike): Keyword arguments for `vectorbtpro.utils.datetime_.to_freq`.
        timestamp_kwargs (KwargsLike): Keyword arguments for `vectorbtpro.utils.datetime_.to_timestamp`.
        humanize_kwargs (KwargsLike): Keyword arguments for `humanize.naturaldelta` when logging.
    Returns:
        None
    """
    ...
    if from_dt is None:
        ...
        from_dt = dt.to_timestamp(from_dt, **timestamp_kwargs)
    freq = dt.to_freq(freq, **freq_kwargs)
    if floor:
        if isinstance(freq, pd.Timedelta):
            from_dt = from_dt.floor(freq)
        else:
            from_dt = freq.rollback(from_dt)
            largest_unit = dt.get_largest_unit(freq)
            from_dt = from_dt.floor(largest_unit)
    seconds = (from_dt + freq - now).total_seconds()
    if log:
        delta = pd.Timedelta(seconds=seconds)
        target_time = now + delta
        if readable:
            readable_delta = humanize.naturaldelta(seconds, **humanize_kwargs)
            readable_datetime = dt.readable_datetime(target_time, freq=freq)
        else:
            readable_delta = delta
            readable_datetime = target_time
        logger.info("Waiting for %s until %s", readable_delta, readable_datetime)
    if seconds > 0:
        time.sleep(seconds)
