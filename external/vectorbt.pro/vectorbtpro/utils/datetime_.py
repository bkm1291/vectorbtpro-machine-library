...
def get_largest_unit(freq: tp.FrequencyLike) -> tp.PandasTimedelta:
    """Get largest time unit that divides the given frequency.

    Args:
        freq (FrequencyLike): Frequency representation (string, offset, or timedelta).

    Returns:
        PandasTimedelta: Largest time unit that divides the given frequency.
    """
    timedelta = to_timedelta(freq, approximate=True)
    for unit in (
        pd.Timedelta(days=1),
        ...
        if timedelta % unit == 0:
            return unit
    return pd.Timedelta(nanoseconds=1)
