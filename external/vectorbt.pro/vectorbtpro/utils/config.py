...
class Configured(HasSettings, Cacheable, Comparable, Pickleable, Prettified, Chainable, metaclass=MetaConfigured):
    ...
    * "custom": Expects custom provided keys.
    ...
    def digest(self, incl_settings: bool = False, **kwargs) -> str:
        """Return the hash digest of the object's serialized form.
        
        Args:
            incl_settings (bool): Whether to include settings in the hash.
            **kwargs: Keyword arguments for `HasSettings.get_settings` if `incl_settings` is True.
        ...
        if incl_settings:
            ...
            if not isinstance(settings, Pickleable):
                settings = Config(settings)
            return hashlib.blake2b(self.dumps() + settings.dumps(), digest_size=16).hexdigest()
