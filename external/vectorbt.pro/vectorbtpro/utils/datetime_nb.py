...
@register_jitted(cache=True)
def is_weekend_nb(ts: int) -> bool:
    """Determine if a given timestamp falls on a weekend.

    Args:
        ts (int): Timestamp in nanoseconds.

    Returns:
        bool: True if the timestamp is on a weekend (Saturday or Sunday), False otherwise.
    """
    weekday = weekday_nb(ts, zero_start=True)
